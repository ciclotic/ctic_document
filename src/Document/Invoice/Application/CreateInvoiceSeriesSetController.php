...
use CTIC\Document\Invoice\Infrastructure\Controller\InvoiceSeriesSetController;
use CTIC\Document\Invoice\Infrastructure\Form\Type\InvoiceSeriesSetType;
...
class CreateInvoiceSeriesSetController extends CreateResourceController
{
    /**
    ...
    ): void
    {
        ...
        $formRegistry->addType(new InvoiceSeriesSetType(InvoiceSeriesSet::class));
    }
        ...
        if(empty($command->metadata))
        {
            $command->metadata = self::getMetadata(
                ...
                array(
                    ...
                    'driver'        => '',
                    'templates'     => '',
                    'header'        => 'serie de facturas',
                    'route'         => array(
                        'root'      => 'facturas_seriesset',
                        'actions'   => array(
                            'list'  => 'listado',
                            'show'  => 'mostrar',
                            'create'    =>'crear',
                    ...
                    'classes'       => array(
                        'form'      => 'CTIC\Document\Invoice\Infrastructure\Form\Type\InvoiceSeriesSetType'
                    ),
                    'grid'          => array(
                        'actionGroups'  => array(
                            'bulk'  => array(
    ...
    public static function setDefaultCommand(ResourceControllerCommand $command): void
    {
        if(empty($command->newResourceFactory))
        {
            $command->newResourceFactory = new CreateInvoiceSeriesSet();
        }
        if(empty($command->resource))
        {
            $command->resource = InvoiceSeriesSet::class;
        }
        if(empty($command->command))
        {
            $command->command = new InvoiceSeriesSetCommand();
        }
    }

    public static function getResourceControllerClass(): string
    {
        return InvoiceSeriesSetController::class;
    }

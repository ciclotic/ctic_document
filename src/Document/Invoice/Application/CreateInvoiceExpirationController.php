...
use CTIC\Document\Invoice\Infrastructure\Controller\InvoiceExpirationController;
use CTIC\Document\Invoice\Infrastructure\Form\Type\InvoiceExpirationType;
...
class CreateInvoiceExpirationController extends CreateResourceController
{
    /**
    ...
    ): void
    {
        ...
        $formRegistry->addType(new InvoiceExpirationType(InvoiceExpiration::class));
    }
        ...
        if(empty($command->metadata))
        {
            $command->metadata = self::getMetadata(
                ...
                array(
                    ...
                    'driver'        => '',
                    'templates'     => '',
                    'header'        => 'vencimiento de facturas',
                    'route'         => array(
                        'root'      => 'facturas_vencimientos',
                        'actions'   => array(
                            'list'  => 'listado',
                            'show'  => 'mostrar',
                            'create'    =>'crear',
                    ...
                    'classes'       => array(
                        'form'      => 'CTIC\Document\Invoice\Infrastructure\Form\Type\InvoiceExpirationType'
                    ),
                    'grid'          => array(
                        'actionGroups'  => array(
                            'bulk'  => array(
    ...
    public static function setDefaultCommand(ResourceControllerCommand $command): void
    {
        if(empty($command->newResourceFactory))
        {
            $command->newResourceFactory = new CreateInvoiceExpiration();
        }
        if(empty($command->resource))
        {
            $command->resource = InvoiceExpiration::class;
        }
        if(empty($command->command))
        {
            $command->command = new InvoiceExpirationCommand();
        }
    }

    public static function getResourceControllerClass(): string
    {
        return InvoiceExpirationController::class;
    }
